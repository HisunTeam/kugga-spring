...
import static com.hisun.kugga.duke.common.BusinessErrorCodeConstants.MESSAGES_TYPE_ERROR;
...
public class MessageUtil {
    ...
    private static void packageMessageInfo(MessagesRespVO messagesRespVO, List<String> params) {
        // 得到唯一消息key
        MessageTemplateEnum template = null;
        try {
            template = MessageTemplateEnum.valueOf(messagesRespVO.getMessageKey());
        } catch (Exception e) {
            logger.error("get MessageTemplateEnum error,{} not exist",messagesRespVO.getMessageKey(),e);
            throw exception(MESSAGES_TYPE_ERROR);
        }
        ...
        switch (template) {
            ...
            case RECOMMENDATION_INVITE:
                 ...
                 */
                params.add(UserUtil.getUserName(messagesRespVO.getInitiatorFirstName(), messagesRespVO.getInitiatorLastName()));
                amount = Optional.ofNullable(messagesRespVO.getMessageParam().getTaskWriteReportCreate().getAmount()).orElse(BigDecimal.ZERO);
                params.add(amount.toString());
                break;
            case RECOMMENDATION_INVITE_FREE:
                // 推荐信 张三邀请写推荐报告 免费
                params.add(UserUtil.getUserName(messagesRespVO.getInitiatorFirstName(), messagesRespVO.getInitiatorLastName()));
                break;
            case RECOMMENDATION_CALLBACK:
                ...
                params.add(UserUtil.getUserName(messagesRespVO.getInitiatorFirstName(), messagesRespVO.getInitiatorLastName()));
                break;
            case REACH_ACCOUNT_RECOMMENDATION:
                // 您撰写推荐报告，到账$50   to李四
                amount = Optional.ofNullable(messagesRespVO.getMessageParam().getTaskWriteReportCreate().getAmount()).orElse(BigDecimal.ZERO);
                params.add(amount.toString());
                break;
            case RECOMMENDATION_CALLBACK_RECEIVER2:
                // 我为张三写了推荐报告
            ...
            case CHAT_INVITE:
                 ...
                 */
                params.add(UserUtil.getUserName(messagesRespVO.getInitiatorFirstName(), messagesRespVO.getInitiatorLastName()));
                amount = Optional.ofNullable(messagesRespVO.getMessageParam().getTaskChat().getAmount()).orElse(BigDecimal.ZERO);
                params.add(amount.toString());
                break;
            case CHAT_INVITE_FREE:
                //张三邀请与我聊天 免费
                params.add(UserUtil.getUserName(messagesRespVO.getInitiatorFirstName(), messagesRespVO.getInitiatorLastName()));
                break;
            case CHAT_CALLBACK:
                ...
                params.add(UserUtil.getUserName(messagesRespVO.getInitiatorFirstName(), messagesRespVO.getInitiatorLastName()));
                break;
            case REACH_ACCOUNT_CHAT:
                //您同意与他人聊天，到账$50
                amount = Optional.ofNullable(messagesRespVO.getMessageParam().getTaskChat().getAmount()).orElse(BigDecimal.ZERO);
                params.add(amount.toString());
                break;
            case CHAT_CALLBACK_RECEIVER2:
                //您同意与他人聊天  纯文案注释
                // params.add(UserUtil.getUserName(messagesRespVO.getInitiatorFirstName(), messagesRespVO.getInitiatorLastName()));
                break;
            case JOIN_LEAGUE_ACTIVE_AGREE:
                // 加入公会 后端开发工程师同意-拒绝你加入公会
                params.add(messagesRespVO.getInitiatorLeagueName());
                break;
            case JOIN_LEAGUE_ACTIVE_REJECT:
                //后端开发工程师拒绝你加入公会
                params.add(messagesRespVO.getInitiatorLeagueName());
                break;
            case JOIN_LEAGUE_ACTIVE_EXPIRE:
                //因为本次加入申请过期，[{后端开发工程师}]公会已经拒绝您的申请
                params.add(messagesRespVO.getInitiatorLeagueName());
                break;
            case LEAGUE_CREATE_AUTH_INVITE:
                //{张三}接受了我创建公会的邀请，公会已正式成立，快去完成公会认证吧
                params.add(UserUtil.getUserName(messagesRespVO.getInitiatorFirstName(), messagesRespVO.getInitiatorLastName()));
                break;
            case LEAGUE_CREATE_RULE_INVITE:
                // {后端开发工程师}公会已完成公会认证，快去设置公会规则吧
                params.add(messagesRespVO.getInitiatorLeagueName());
                break;
            case LEAGUE_FIRST_JOIN_INVITE:
                // 第一次加入公会,您是第一个受邀加入公会[{后端开发工程师}]的会员，并获得{10}美元的奖金。
                params.add(messagesRespVO.getInitiatorLeagueName());
                amount = Optional.ofNullable(messagesRespVO.getMessageParam().getJoinLeagueMessageVO().getAmount()).orElse(BigDecimal.ZERO);
                params.add(amount.toString());
                break;
            case LEAGUE_FIRST_JOIN_INVITE_FREE:
                // 您是第一个受邀加入公会[后端开发工程师]的会员
                params.add(messagesRespVO.getInitiatorLeagueName());
                break;
            case LEAGUE_JOIN_INVITE:
                ...
                break;
            case INVITE_LINK_EXPIRE_CREATE_LEAGUE:
                break;
            case LEAGUE_SUBSCRIBE_RENEWAL:
                ...
                amount = Optional.ofNullable(messagesRespVO.getMessageParam().getLeagueSubscribeVO().getAmount()).orElse(BigDecimal.ZERO);
                params.add(amount.toString());
                break;
            case LEAGUE_SUBSCRIBE_NO_BALANCE_QUIT:
                // 由于帐户余额不足，您对｛xxx｝公会的订阅已暂停。您将无法访问｛xxx｝公会及其福利。
                params.add(messagesRespVO.getInitiatorLeagueName());
                params.add(messagesRespVO.getInitiatorLeagueName());
                break;
            case LEAGUE_RED_PACKAGE:
                // 获得后端开发工程师公会100$的红包  You have received ${} from the {} Guild.
                amount = Optional.ofNullable(messagesRespVO.getMessageParam().getAmount()).orElse(BigDecimal.ZERO);
                params.add(amount.toString());
